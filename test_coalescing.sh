...
echo ""
echo "=== Running Q19 with liquid-evict @ 256MB (coalescing should activate) ==="
echo "--- Iteration 1 (cold cache, fills + spills to disk) ---"
./target/release/in_process \
  --manifest benchmark/clickbench/manifest.json \
  --bench-mode liquid-evict \
  --max-memory-mb 256 \
  --query-index 19 \
  --iteration 3 \
  --reset-cache \
  --output /tmp/coalesce_q19_evict.json

echo ""
echo "Results saved to /tmp/coalesce_q19_evict.json"
echo ""
...
echo "=== Running Q7 with liquid-evict @ 64MB (smaller budget, more spill) ==="
./target/release/in_process \
  --manifest benchmark/clickbench/manifest.json \
  --bench-mode liquid-evict \
  --max-memory-mb 64 \
  ...
  --iteration 3 \
  --reset-cache \
  --output /tmp/coalesce_q7_evict.json

echo ""
echo "=== Summary ==="
echo "Q19 liquid-evict (coalesced):"
python3 -c "
import json
with open('/tmp/coalesce_q19_evict.json') as f:
    d = json.load(f)
...
" 2>/dev/null || echo "  (parse output manually: cat /tmp/coalesce_q19_evict.json | python3 -m json.tool)"
